...
const Leave = () => {
  ...
  const handleApplyLeave = () => {
    ...
    if (editIndex !== null) {
      ...
      const maxId = leaveRequests.length > 0 
  ...
  const LeaveStatus = (leaveId: string, newStatus: string) => {
                ...
                <tr key={leave.leaveId} className="border-t">
                  ...
                  <td
                    className={`p-3 border border-black  font-bold ${
                      leave.status === "Pending" ? "text-yellow-500" : ""}${leave.status === "Approved" ? "text-green-500" : ""}${leave.status === "Rejected" ? "text-red-500" : ""}`}>
          ...
          <tbody>
            {leaveRequests.map((leave, index) => (
              <tr key={leave.leaveId} className="border-t">
                ...
                <td className="p-3 border">{leave.absenceHours}</td>
                <td className="p-3 border">{leave.reason}</td>
                <td className={`p-3 border border-black font-bold ${leave.status === "Pending" ? "text-yellow-500" : ""}${leave.status === "Approved" ? "text-green-500" : ""}${leave.status === "Rejected" ? "text-red-500" : ""}`}>
                  {editIndex === index ? (
                    <select
